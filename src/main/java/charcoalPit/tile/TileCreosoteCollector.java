...
public class TileCreosoteCollector extends TileEntity implements ITickable{
	...
	public void update() {
		if(tick<20){
			...
			tick=0;
			//collect creosote
			if(creosote.getFluidAmount()<creosote.getCapacity()&&this.worldObj.getTileEntity(this.pos.offset(EnumFacing.UP))instanceof TileActivePile){
				TileActivePile up=(TileActivePile)this.worldObj.getTileEntity(this.pos.offset(EnumFacing.UP));
			...
			//output creosote
			if(creosote.getFluidAmount()>0&&this.worldObj.isBlockPowered(this.pos)){
				TileEntity tile=this.worldObj.getTileEntity(this.pos.offset(EnumFacing.DOWN));
				if(tile!=null){
	...
	public void collectCreosote(BlockPos pos, EnumFacing facing, int runs){
		if(this.worldObj.getTileEntity(pos)instanceof TileActivePile){
			TileActivePile up=(TileActivePile)this.worldObj.getTileEntity(pos);
			up.creosote.drain(creosote.fillInternal(up.creosote.getFluid(), true), true);
	...
	public void chanelCreosote(BlockPos pos, EnumFacing facing, int runs){
		if(this.worldObj.getTileEntity(pos)instanceof TileCreosoteCollector){
			TileCreosoteCollector up=(TileCreosoteCollector)this.worldObj.getTileEntity(pos);
			up.creosote.drain(creosote.fillInternal(up.creosote.getFluid(), true), true);
			if(runs>0&&creosote.getFluidAmount()<creosote.getCapacity())
