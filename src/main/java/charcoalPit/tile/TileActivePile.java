		...
		if(!this.worldObj.isRemote){
			...
			if(burnTime>0){
				...
				if(itemsLeft>0){
					...
					this.worldObj.setBlockState(this.pos, isCoke?BlocksRegistry.CokePile.getDefaultState():BlocksRegistry.CharcoalPile.getDefaultState());
				}
			}
			if(creosote.getFluidAmount()>0){
				if(this.worldObj.getTileEntity(this.pos.offset(EnumFacing.DOWN))instanceof TileActivePile){
					TileActivePile down=(TileActivePile)this.worldObj.getTileEntity(this.pos.offset(EnumFacing.DOWN));
	...
	public void checkValid(){
		if(!isValid){
			...
			for(EnumFacing facing:neighbors){
				IBlockState block=this.worldObj.getBlockState(this.pos.offset(facing));
				if((!isValidBlock(block))||
						(!block.isSideSolid(this.worldObj, this.pos.offset(facing), facing.getOpposite()))||
						block.getBlock().isFlammable(this.worldObj, this.pos.offset(facing), facing.getOpposite())){
					valid=false;
					break;
				}
			}
			if(valid){
				...
				if(invalidTicks<100){
					...
					for(EnumFacing facing:neighbors){
						//set fire
						IBlockState block=this.worldObj.getBlockState(this.pos.offset(facing));
						if(block.getBlock().isAir(block, this.worldObj, this.pos.offset(facing))||
								block.getBlock().isReplaceable(this.worldObj, this.pos.offset(facing))){
							this.worldObj.setBlockState(this.pos.offset(facing), Blocks.FIRE.getDefaultState());
						}
					}
				}else{
